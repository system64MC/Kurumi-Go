...
func getWTSample(op *Operator, x float64) float64 {
	return interpolate(x * float64(len(op.Wavetable)) * float64(op.Mult) + (float64(op.Phase) * float64(len(op.Wavetable)) + op.getPhase() * float64(len(op.Wavetable))), op)
}
...
func Synthesize() {
	...
	myTmp := make([]float64, 0)
	for x := 0; x < int(myLen*oversample); x++ {
		myTmp = append(myTmp, fm(float64(x)))
	}
...
func Wavetable(userdata unsafe.Pointer, stream *C.Uint8, length C.int) {
	...
	for i := 0; i < n; i ++ {
		...
		if(SynthContext.SongPlaying && len(WaveOutput) > 0 && WaveOutput != nil) {
			sample = WaveOutput[ind % len(WaveOutput)]
		}
